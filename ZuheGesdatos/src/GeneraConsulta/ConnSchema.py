...
class ConnSchema():
    ...
    def GetTables(self,schema):
        ret = self.connection.ExecuteQuery("SELECT table_name FROM information_schema.tables WHERE table_schema != 'pg_catalog' AND table_schema != 'information_schema' AND table_catalog = '"+schema+"'")
        for i in range(len(ret)):
            ret[i] = str(ret[i][0])
        return ret

    def GetColumns(self,schema,table):
        ret = self.connection.ExecuteQuery("SELECT column_name FROM information_schema.columns WHERE table_schema != 'pg_catalog' AND table_schema != 'information_schema' AND table_catalog ='"+schema+"' AND table_name ='"+table+"'")
        for i in range(len(ret)):
            ret[i] = str(ret[i][0])
        return ret
